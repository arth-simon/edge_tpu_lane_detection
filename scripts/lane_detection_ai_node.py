...
import numpy as np
import rospkg
from lane_detection_ai.model_wrapper import LaneDetectionAiModel
...
class LaneDetectionAiNode(node_base.NodeBase):
    ...
    def __init__(self):
        ...
        self.model = LaneDetectionAiModel(self.package_path,
    ...
    def camera_image_cb(self, camera_image_msg: sensor_msgs.msg.Image):
        ...
        if self.param.active:
            ...
            with Timer(name="debug_image", filter_strength=40):
                if self.param.debug:
                    debug_image = self.birds_eyed.undistorted_to_birdseyedview(camera_image.copy())
                    if len(left_lane) > 0:
                        for coord in self.coord_trans.world_to_bird(left_lane).astype(int):
                            cv2.circle(debug_image, coord, 5, (255, 0, 0), -1)
                    if len(center_lane) > 0:
                        for coord in self.coord_trans.world_to_bird(center_lane).astype(int):
                            cv2.circle(debug_image, coord, 5, (0, 255, 0), -1)
                    if len(right_lane) > 0:
                        for coord in self.coord_trans.world_to_bird(right_lane).astype(int):
